...
@section('title', 'Edit Shift Schedule - SENA.ERP')

@section('content')
<div class="container-fluid py-4">
    <div class="row mb-4">
        <div class="col-12">
            <h2 class="mb-0"><i class="fas fa-edit me-2"></i>Edit Shift Schedule</h2>
    ...
    <div class="row">
        <div class="col-lg-9">
            <form action="{{ route('hr.shift-schedules.update', $shiftSchedule) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="card-body">
                                ...
                                <select class="form-select @error('employee_id') is-invalid @enderror" 
                                    ...
                                    @foreach($employees as $employee)
                                        <option value="{{ $employee->id }}" 
                                                {{ old('employee_id', $shiftSchedule->employee_id) == $employee->id ? 'selected' : '' }}>
                                ...
                                <select class="form-select @error('shift_id') is-invalid @enderror" 
                                    ...
                                    @foreach($shifts as $shift)
                                        <option value="{{ $shift->id }}" 
                                                {{ old('shift_id', $shiftSchedule->shift_id) == $shift->id ? 'selected' : '' }}
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control @error('schedule_date') is-invalid @enderror" 
                                       ...
                                       value="{{ old('schedule_date', $shiftSchedule->schedule_date->format('Y-m-d')) }}" required>
                                @error('schedule_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" 
                                    ...
                                    <option value="Scheduled" {{ old('status', $shiftSchedule->status) == 'Scheduled' ? 'selected' : '' }}>Scheduled</option>
                                    <option value="Confirmed" {{ old('status', $shiftSchedule->status) == 'Confirmed' ? 'selected' : '' }}>Confirmed</option>
                                    <option value="Cancelled" {{ old('status', $shiftSchedule->status) == 'Cancelled' ? 'selected' : '' }}>Cancelled</option>
                                    <option value="Swapped" {{ old('status', $shiftSchedule->status) == 'Swapped' ? 'selected' : '' }}>Swapped</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="is_overtime" name="is_overtime" value="1"
                                       {{ old('is_overtime', $shiftSchedule->is_overtime) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror" 
                                      id="notes" name="notes" rows="3">{{ old('notes', $shiftSchedule->notes) }}</textarea>
