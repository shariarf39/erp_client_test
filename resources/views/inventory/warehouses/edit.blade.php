        ...
        <div>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active">Edit: {{ $warehouse->warehouse_name }}</li>
                </ol>
            </nav>
        </div>
        <div>
            <a href="{{ route('inventory.warehouses.show', $warehouse) }}" class="btn btn-info">
                ...
                <div class="card-body p-4">
                    <form action="{{ route('inventory.warehouses.update', $warehouse) }}" method="POST">
                                ...
                                <select class="form-select @error('manager_id') is-invalid @enderror" 
                                    ...
                                    @foreach($employees as $employee)
                                        <option value="{{ $employee->id }}" 
                                                {{ old('manager_id', $warehouse->manager_id) == $employee->id ? 'selected' : '' }}>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            <a href="{{ route('inventory.warehouses.show', $warehouse) }}" class="btn btn-secondary">
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm border-0 mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <div class="h6">{{ $warehouse->warehouse_code }}</div>
                    </div>
                    <div class="mb-3">
                        <div class="text-muted small">Warehouse Name</div>
                        <div class="h6">{{ $warehouse->warehouse_name }}</div>
                    </div>
                    <div class="mb-3">
                        <div class="text-muted small">City</div>
                        <div>{{ $warehouse->city ?? 'Not specified' }}</div>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="h5 text-primary">{{ $warehouse->stocks->count() }}</div>
                    </div>
                    <div class="mb-3">
                        <div class="text-muted small">Total Quantity</div>
                        <div class="h5 text-success">{{ number_format($warehouse->stocks->sum('quantity'), 2) }}</div>
                    </div>
                </div>
            </div>
            ...
            <div class="card shadow-sm border-0">
                <div class="card-header bg-warning text-dark py-3">
                    <h5 class="mb-0">
                        <i class="bi bi-lightbulb me-2"></i>Edit Tips
                    </h5>
                </div>
                <div class="card-body">
                    <ul class="mb-0 ps-3">
                        <li class="mb-2"><strong>Warehouse Code:</strong> Must be unique across all warehouses</li>
                        <li class="mb-2"><strong>Manager:</strong> Select from active employees only</li>
                        <li class="mb-2"><strong>Inactive Status:</strong> Will hide warehouse from dropdowns</li>
                        <li><strong>Cannot Delete:</strong> If warehouse has stock records</li>
                    </ul>
                </div>
            </div>
        </div>
